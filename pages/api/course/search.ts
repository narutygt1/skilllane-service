...
export default async function handler(req: NextApiRequest, res: NextApiResponse<IResponseData>) {
	...
	if (req.method == "GET") {
		...
		const searchName = name || undefined;
		const searchTime = time ? new Date(time as string) : undefined;
		...
		const courseRes =
			(await CourseModel.find({
				$and: [
					name
						? { name: { $regex: new RegExp(".*" + (searchName as string).toLowerCase() + ".*", "i") } }
						: {},
					time ? { start_time: { $lte: searchTime }, end_time: { $gte: searchTime } } : {},
				],
			}).populate([
				{
					path: "category",
